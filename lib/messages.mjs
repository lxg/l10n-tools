...
function getFileEntries(fileName, file, instanceName) {
    ...
    walk.full(ast, node => {
        if (node.type === "CallExpression" &&
            ...
            node.callee.object.name === instanceName)
        {
            const position = node.callee.loc.start

            if (node.callee.property.name === "t")
            {
                validateSignature(fileName, position, "L10n.x", node.arguments, ["string"])
            ...
            {
                validateSignature(fileName, position, "L10n.n", node.arguments, ["string", "string", "number"])
...
function validateSignature(fileName, position, funcName, args, expected) {
    ...
    expected.forEach((type, i) => {
        if (args[i].type !== "Literal" || typeof(args[i].value) !== type)
